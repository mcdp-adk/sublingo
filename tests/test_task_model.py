...
def test_task_manager_resolves_output_dir_from_config_when_param_missing(
    tmp_path: Path,
) -> None:
    manager = TaskManager(
...
@pytest.mark.asyncio
async def test_task_manager_workflow_runner_uses_resolved_output_dir(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    manager = TaskManager(
    ...
    task = TaskInfo(task_type=TaskType.WORKFLOW, params={"url": "https://example.com"})
    captured: dict[str, Any] = {}

    async def fake_run_workflow(*args: Any, **kwargs: Any) -> Any:
        captured.update(kwargs)
        return object()

    monkeypatch.setattr("sublingo.gui.models.task.run_workflow", fake_run_workflow)

    runner = manager._build_runner(task)
